...
@ConsoleCommand(usage = "bot <Create|Remove|List> [ID/Name]", aliases = { "bot" })
public class CommandBot implements IConsoleCommandExecutor {
	...
	public ConsoleCommandResult onCommand(String[] args) {
		if(args.length > 0) {
			if(args[0].equalsIgnoreCase("create") || args[0].equalsIgnoreCase("c")) {
				...
				AuthManager.saveId(id, keyPair.getPublic(), name, EnumAuthManagerType.BOT);
				...
				try (DataOutputStream outputStream = new DataOutputStream(Files.newOutputStream(Paths.get(String.format("wuffy/%s#bot.key", id.toString()))))) {
				...
				if(set.isEmpty())
					...
					set.stream()
								.filter(id -> AuthManager.getType(id) == EnumAuthManagerType.BOT)
