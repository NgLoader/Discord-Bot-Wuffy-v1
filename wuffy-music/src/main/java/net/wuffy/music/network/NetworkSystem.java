...
import net.wuffy.music.MusicConfig;
...
public class NetworkSystem extends AbstractClient implements ITickable {

	private final List<NetworkManager> networkManager = new ArrayList<NetworkManager>();

	public NetworkSystem start(MusicConfig config) throws SSLException {
		...
		this.start(new InetSocketAddress(config.masterAddress, config.masterPort), 0, new ChannelInitializer<Channel>() {
