...
public class CommandRule34 extends Command {
	...
	public void sendImage(WuffyMessageRecivedEvent event, ArgumentBuffer args, String locale) {
		try {
			...
			Response response = WebRequestBuilder.request(new Request.Builder()
					.url(String.format("%s%s", CommandRule34.RULE34_URL, args.getSize() > 0 ? String.join("_", Arrays.copyOfRange(args.getArguments(), 0, args.getSize())) : "random"))
					.build());
