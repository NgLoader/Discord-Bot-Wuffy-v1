		...
		if(args.getSize() > 0) {
			...
			if(args.get(0).equalsIgnoreCase("list")) {
				...
				if(args.get(0).equalsIgnoreCase("add")) {
					if(prefixes.size() < 10) {
						if(args.get(1).length() < 11) {
							if(prefixes.stream().noneMatch(prefix -> prefix.equalsIgnoreCase(args.get(1)))) {
								guild.addPrefix(args.get(1).toLowerCase());
								this.sendMessage(event, MessageType.SUCCESS, i18n.format(TranslationKeys.MESSAGE_PREFIX_ADDED, locale, "%p", args.get(1).toLowerCase()));
							} else
...
//					if(prefixes.size() > 1) {
						if(prefixes.stream().anyMatch(prefix -> prefix.equalsIgnoreCase(args.get(1)))) {
							guild.removePrefix(args.get(1).toLowerCase());
							this.sendMessage(event, MessageType.SUCCESS, i18n.format(TranslationKeys.MESSAGE_PREFIX_REMOVED, locale, "%p", args.get(1).toLowerCase()));
						} else
