...
@Command(aliases = { "lewd" })
@CommandConfig(category = CommandCategory.IMAGE)
public class CommandRRALewd extends BotCommand {
	...
	public void execute(WuffyMessageRecivedEvent event, String[] args) {
		...
		if(event.getMember(WuffyMember.class).hasPermission(event.getTextChannel(), PermissionKeys.COMMAND_LEWD)) {
			...
			String picture = RRABuilder.getRandom(EnumRRATypes.LEWD);
			...
			this.replay(event, MessageType.PERMISSION, i18n.format(TranslationKeys.MESSAGE_NO_PERMISSION, locale,
					"%p", PermissionKeys.COMMAND_LEWD.key));
	}
}