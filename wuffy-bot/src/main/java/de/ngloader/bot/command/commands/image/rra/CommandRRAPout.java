...
@Command(aliases = { "pout" })
@CommandConfig(category = CommandCategory.IMAGE)
public class CommandRRAPout extends BotCommand {
	...
	public void execute(WuffyMessageRecivedEvent event, String[] args) {
		...
		if(event.getMember(WuffyMember.class).hasPermission(event.getTextChannel(), PermissionKeys.COMMAND_POUT)) {
			...
			String picture = RRABuilder.getRandom(EnumRRATypes.POUT);
			...
			this.replay(event, MessageType.PERMISSION, i18n.format(TranslationKeys.MESSAGE_NO_PERMISSION, locale,
					"%p", PermissionKeys.COMMAND_POUT.key));
	}
}