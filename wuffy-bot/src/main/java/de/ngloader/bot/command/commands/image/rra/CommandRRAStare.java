...
@Command(aliases = { "stare" })
@CommandConfig(category = CommandCategory.IMAGE)
public class CommandRRAStare extends BotCommand {
	...
	public void execute(WuffyMessageRecivedEvent event, String[] args) {
		...
		if(event.getMember(WuffyMember.class).hasPermission(event.getTextChannel(), PermissionKeys.COMMAND_STARE)) {
			...
			String picture = RRABuilder.getRandom(EnumRRATypes.STARE);
			...
			this.replay(event, MessageType.PERMISSION, i18n.format(TranslationKeys.MESSAGE_NO_PERMISSION, locale,
					"%p", PermissionKeys.COMMAND_STARE.key));
	}
}