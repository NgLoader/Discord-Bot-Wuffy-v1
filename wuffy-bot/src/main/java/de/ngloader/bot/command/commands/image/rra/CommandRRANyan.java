...
@Command(aliases = { "nyan" })
@CommandConfig(category = CommandCategory.IMAGE)
public class CommandRRANyan extends BotCommand {
	...
	public void execute(WuffyMessageRecivedEvent event, String[] args) {
		...
		if(event.getMember(WuffyMember.class).hasPermission(event.getTextChannel(), PermissionKeys.COMMAND_NYAN)) {
			...
			String picture = RRABuilder.getRandom(EnumRRATypes.NYAN);
			...
			this.replay(event, MessageType.PERMISSION, i18n.format(TranslationKeys.MESSAGE_NO_PERMISSION, locale,
					"%p", PermissionKeys.COMMAND_NYAN.key));
	}
}