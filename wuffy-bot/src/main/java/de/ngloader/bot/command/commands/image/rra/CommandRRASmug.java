...
@Command(aliases = { "smug" })
@CommandConfig(category = CommandCategory.IMAGE)
public class CommandRRASmug extends BotCommand {
	...
	public void execute(WuffyMessageRecivedEvent event, String[] args) {
		...
		if(event.getMember(WuffyMember.class).hasPermission(event.getTextChannel(), PermissionKeys.COMMAND_SMUG)) {
			...
			String picture = RRABuilder.getRandom(EnumRRATypes.SMUG);
			...
			this.replay(event, MessageType.PERMISSION, i18n.format(TranslationKeys.MESSAGE_NO_PERMISSION, locale,
					"%p", PermissionKeys.COMMAND_SMUG.key));
	}
}