...
public class CommandRRACry extends BotCommand {
	...
	public void execute(WuffyMessageRecivedEvent event, String[] args) {
		...
		if(event.getMember(WuffyMember.class).hasPermission(event.getTextChannel(), PermissionKeys.COMMAND_CRY)) {
			...
			this.replay(event, MessageType.PERMISSION, i18n.format(TranslationKeys.MESSAGE_NO_PERMISSION, locale,
					"%p", PermissionKeys.COMMAND_CRY.key));
	}
}