...
//		event.getChannel().sendMessage(event.getCore().getI18n().format(TranslationKeys.MESSAGE_INVITE, event.getMember(WuffyMember.class).getLocale())).queue();
		event.getAuthor().openPrivateChannel().complete().sendMessage(event.getCore().getI18n().format(TranslationKeys.MESSAGE_INVITE, event.getMember(WuffyMember.class).getLocale())).queue();
	}
}