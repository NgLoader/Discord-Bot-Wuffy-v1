...
public class PacketBuffer extends ByteBuf {
	...
	/**
	 * Returns the size of varint in bytes
	...
	public int readVarInt() {
		...
		while (true) {
			...
			out |= (in & 0x7F) << (bytes++ * 7);
			if (bytes > 5)
				throw new DecoderException("Attempt to read int bigger than 5 bytes");
			if ((in & 0x80) != 0x80)
				break;
		}
		return out;
	...
	public long readVarLong() {
		...
		while (true) {
			...
			if (bytes > 10)
				throw new DecoderException("Attempt to read long bigger than 10 bytes");
			if ((in & 0x80) != 0x80)
				break;
		}
		return out;
	...
	/**
	 * Reads {@link Enum} of given class
	 * 
	...
	/**
	 * Writes {@link InetSocketAddress}
	 * 
	 * @param address {@link InetSocketAddress} to write
	 */
	public void writeAddress(InetSocketAddress address) {
		this.writeByteArray(address.getAddress().getAddress());
