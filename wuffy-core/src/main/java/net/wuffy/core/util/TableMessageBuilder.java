...
public class TableMessageBuilder {
	...
	public String build() {
		...
		var spaces = new HashMap<Integer, Integer[]>();
		...
		for(int currentlyTable : this.table.keySet()) {
			...
			for(int line : this.table.get(currentlyTable).keySet()) {
				...
				lines.put(currentlyTable, String.format("%s%s%s%s",
						...
						generateSpace(spaces.get(line)[0] - tableMessageInfo.getPrefix().length()) + tableMessageInfo.getPrefix(),
						generateSpace(spaces.get(line)[1] - tableMessageInfo.getContent().length()) + tableMessageInfo.getContent(),
						generateSpace(spaces.get(line)[2] - tableMessageInfo.getSuffix().length()) + tableMessageInfo.getSuffix()));
			}
		}
