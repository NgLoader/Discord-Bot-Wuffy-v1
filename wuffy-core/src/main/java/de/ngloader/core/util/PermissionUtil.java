...
public class PermissionUtil {
	 ...
	 * permission sense (kick/ban/modify perms). This only checks the Role-Position
	 * and does not check the actual permission (kick/ban/manage_role/...)
	 *
	 ...
	 *                                  {@code null} or the provided entities are
	 *                                  not from the same guild
	 *
	...
	public static boolean canInteract(Member issuer, Member target) {
		...
		if (guild.getIdLong() != target.getGuild().getIdLong())
	 ...
	 * sense (kick/ban/modify perms). This only checks the Role-Position and does
	 * not check the actual permission (kick/ban/manage_role/...)
	 *
	...
	public static boolean canInteract(Role issuer, Role target) {
		...
		if (issuer.getGuild().getIdLong() != target.getGuild().getIdLong())
			throw new IllegalArgumentException("The 2 Roles are not from same Guild!");
		return target.getPosition() < issuer.getPosition();
	}

	/**
	 * Check whether the provided {@link net.dv8tion.jda.core.entities.Member
	 * Member} can use the specified {@link net.dv8tion.jda.core.entities.Emote
	 * Emote}.
	 *
	 * <p>
	 * If the specified Member is not in the emote's guild or the emote provided is
	 * fake this will return false. Otherwise it will check if the emote is
	 * restricted to any roles and if that is the case if the Member has one of
	 * these.
	 *
	 * <p>
	 * In the case of an {@link net.dv8tion.jda.core.entities.Emote#isAnimated()
	 * animated} Emote, this will check if the issuer is the currently logged in
	 * account, and then check if the account has
	 * {@link net.dv8tion.jda.core.entities.SelfUser#isNitro() nitro}, and return
	 * false if it doesn't. <br>
	 * For other accounts, this method will not take into account whether the emote
	 * is animated, as there is no real way to check if the Member can interact with
	 * them.
	 *
	 * <br>
	...
	public static boolean canInteract(Member issuer, Emote emote) {
		...
		if (issuer.getGuild().getIdLong() != emote.getGuild().getIdLong())
		...
		// because BOT accounts cannot have nitro, and have access to animated Emotes
		// naturally.
		if (emote.isAnimated() && !issuer.getUser().isBot()) {
			// This is a currently logged in client, meaning we can check if they have nitro
			// or not.
			// If this isn't the currently logged in account, we just check it like a normal
			// emote,
	...
	/**
	 * Checks whether the specified {@link net.dv8tion.jda.core.entities.Emote
	 * Emote} can be used by the provided {@link net.dv8tion.jda.core.entities.User
	 * User} in the {@link net.dv8tion.jda.core.entities.MessageChannel
	 * MessageChannel}.
	 *
	 ...
	 *                                  not from the same guild
	 *
	 * @return True, if the issuer can interact with the emote within the specified
	 *         MessageChannel
	 */
	...
	/**
	 * Checks to see if the {@link net.dv8tion.jda.core.entities.Member Member} has
	 * the specified {@link net.dv8tion.jda.core.Permission Permissions} in the
	 * specified {@link net.dv8tion.jda.core.entities.Guild Guild}. This method
	 * properly deals with Owner status.
	 *
	 * <p>
	 * <b>Note:</b> this is based on effective permissions, not literal permissions.
	 * If a member has permissions that would enable them to do something without
	 * the literal permission to do it, this will still return true. <br>
	 * Example: If a member has the
	 * {@link net.dv8tion.jda.core.Permission#ADMINISTRATOR} permission, they will
	 * be able to {@link net.dv8tion.jda.core.Permission#MANAGE_SERVER} as well,
	 * even without the literal permissions.
	 *
	 * @param member      The {@link net.dv8tion.jda.core.entities.Member Member}
	 *                    whose permissions are being checked.
	 * @param permissions The {@link net.dv8tion.jda.core.Permission Permissions}
	 *                    being checked for.
	 *
	 * @throws IllegalArgumentException if any of the provided parameters is null
	 *
	 * @return True - if the {@link net.dv8tion.jda.core.entities.Member Member}
	 *         effectively has the specified {@link net.dv8tion.jda.core.Permission
	 *         Permissions}.
	 */
	 ...
	 * Checks to see if the {@link net.dv8tion.jda.core.entities.Member Member} has
	 * the specified {@link net.dv8tion.jda.core.Permission Permissions} in the
	 * specified {@link net.dv8tion.jda.core.entities.Channel Channel}. This method
	 * properly deals with {@link net.dv8tion.jda.core.entities.PermissionOverride
	 ...
	 * Example: If a member has the
	 * {@link net.dv8tion.jda.core.Permission#ADMINISTRATOR} permission, they will
	 * be able to {@link net.dv8tion.jda.core.Permission#MESSAGE_WRITE} in every
	 * channel.
	 *
	 * @param member      The {@link net.dv8tion.jda.core.entities.Member Member}
	 *                    whose permissions are being checked.
	 * @param channel     The {@link net.dv8tion.jda.core.entities.Channel Channel}
	 *                    being checked.
	 * @param permissions The {@link net.dv8tion.jda.core.Permission Permissions}
	 *                    being checked for.
	 *
...
//            return true; // can be removed as getEffectivePermissions calculates these cases in
	 ...
	 * this {@link net.dv8tion.jda.core.entities.Member Member} in this
	 * {@link net.dv8tion.jda.core.entities.Guild Guild}. This can be used in
	 * conjunction with {@link net.dv8tion.jda.core.Permission#getPermissions(long)
	 * Permission.getPermissions(int)} to easily get a list of all
	 * {@link net.dv8tion.jda.core.Permission Permissions} that this member has in
	 * this {@link net.dv8tion.jda.core.entities.Guild Guild}.
	 *
	 * <p>
	 * <b>This only returns the Guild-level permissions!</b>
	 *
	 * @param member The {@link net.dv8tion.jda.core.entities.Member Member} whose
	 ...
	 *                                  not from the same guild
	 *
	 * @return The {@code long} representation of the literal permissions that this
	 *         {@link net.dv8tion.jda.core.entities.Member Member} has in this
	 ...
	 * Gets the {@code long} representation of the effective permissions allowed for
	 * this {@link net.dv8tion.jda.core.entities.Member Member} in this
	 * {@link net.dv8tion.jda.core.entities.Channel Channel}. This can be used in
	 * conjunction with {@link net.dv8tion.jda.core.Permission#getPermissions(long)
	 * Permission.getPermissions(long)} to easily get a list of all
	 * {@link net.dv8tion.jda.core.Permission Permissions} that this member can use
	 * in this {@link net.dv8tion.jda.core.entities.Channel Channel}. <br>
	 * This functions very similarly to how
	 * {@link net.dv8tion.jda.core.entities.Role#getPermissionsRaw()
	 ...
	 *         this {@link net.dv8tion.jda.core.entities.Member Member} has in this
	...
	public static long getEffectivePermission(Channel channel, Member member) {
		...
		/*
		 * // currently discord doesn't implicitly grant permissions that the user can
		 * grant others // so instead the user has to explicitly make an override to
		 * grant them the permission in order to be granted that permission // yes this
		 * makes no sense but what can i do, the devs don't like changing things
		 * apparently... // I've been told half a year ago this would be changed but
		 * nothing happens // so instead I'll just bend over for them so people get
		 * "correct" permission checks... // // only time will tell if something happens
		 * and I can finally re-implement this section wew final long managePerms =
		 * Permission.MANAGE_PERMISSIONS.getRawValue(); final long manageChannel =
		 * Permission.MANAGE_CHANNEL.getRawValue(); if ((permission & (managePerms |
		 * manageChannel)) != 0) { // In channels, MANAGE_CHANNEL and MANAGE_PERMISSIONS
		 * grant full text/voice permissions permission |=
		 * Permission.ALL_TEXT_PERMISSIONS | Permission.ALL_VOICE_PERMISSIONS; }
		 */
	}

	/**
	 * Gets the {@code long} representation of the effective permissions allowed for
	 * this {@link net.dv8tion.jda.core.entities.Role Role} in this
	 * {@link net.dv8tion.jda.core.entities.Channel Channel}. This can be used in
	 * conjunction with {@link net.dv8tion.jda.core.Permission#getPermissions(long)
	 * Permission.getPermissions(long)} to easily get a list of all
	 * {@link net.dv8tion.jda.core.Permission Permissions} that this role can use in
	 * this {@link net.dv8tion.jda.core.entities.Channel Channel}.
	 *
	 * @param channel The {@link net.dv8tion.jda.core.entities.Channel Channel} in
	 *                which permissions are being checked.
	 * @param role    The {@link net.dv8tion.jda.core.entities.Role Role} whose
	 *                permissions are being checked.
	 *
	 * @throws IllegalArgumentException if any of the provided parameters is
	 *                                  {@code null} or the provided entities are
	 ...
	 *
	 * @return The {@code long} representation of the effective permissions that
	 *         this {@link net.dv8tion.jda.core.entities.Role Role} has in this
	 *         {@link net.dv8tion.jda.core.entities.Channel Channel}
	 */
	public static long getEffectivePermission(Channel channel, Role role) {
		...
		if (guild.getIdLong() != role.getGuild().getIdLong())
	 ...
	 * {@link net.dv8tion.jda.core.entities.Member Member} in its hosting
	 ...
	 * Permissions like {@link net.dv8tion.jda.core.Permission#ADMINISTRATOR
	 * Permission.ADMINISTRATOR} do not grant other permissions in this value.
	 *
	 * @param member The non-null {@link net.dv8tion.jda.core.entities.Member
	 ...
	 * Retrieves the explicit permissions of the specified
	 * {@link net.dv8tion.jda.core.entities.Member Member} in its hosting
	 * {@link net.dv8tion.jda.core.entities.Guild Guild} and specific
	 * {@link net.dv8tion.jda.core.entities.Channel Channel}. <br>
	 * This method does not calculate the owner in. <b>Allowed permissions override
	 * denied permissions of {@link net.dv8tion.jda.core.entities.PermissionOverride
	 ...
	 * Permission.ADMINISTRATOR} do not grant other permissions in this value.
	 * <p>
	 * This factor in all {@link net.dv8tion.jda.core.entities.PermissionOverride
	 * PermissionOverrides} that affect this member and only grants the ones that
	 * are explicitly given.
	 *
	 * @param channel The target channel of which to check
	 *                {@link net.dv8tion.jda.core.entities.PermissionOverride
	 *                PermissionOverrides}
	 * @param member  The non-null {@link net.dv8tion.jda.core.entities.Member
	 *                Member} for which to get implicit permissions
	 *
	 * @throws IllegalArgumentException If any of the arguments is {@code null} or
	 *                                  the specified entities are not from the same
	...
	/**
	 * Retrieves the explicit permissions of the specified
	 * {@link net.dv8tion.jda.core.entities.Role Role} in its hosting
	 * {@link net.dv8tion.jda.core.entities.Guild Guild} and specific
	 * {@link net.dv8tion.jda.core.entities.Channel Channel}. <br>
	 ...
	 *
	 * <p>
	 * All permissions returned are explicitly granted to this Role. <br>
	 * Permissions like {@link net.dv8tion.jda.core.Permission#ADMINISTRATOR
	 * Permission.ADMINISTRATOR} do not grant other permissions in this value.
	 * <p>
	 * This factor in existing
	 * {@link net.dv8tion.jda.core.entities.PermissionOverride PermissionOverrides}
	 * if possible.
	 *
	 * @param channel The target channel of which to check
	 *                {@link net.dv8tion.jda.core.entities.PermissionOverride
	 *                PermissionOverrides}
	 * @param role    The non-null {@link net.dv8tion.jda.core.entities.Role Role}
	...
	private static void getExplicitOverrides(Channel channel, Member member, AtomicLong allow, AtomicLong deny) {
		...
		if (override != null) {
			...
			// new denied bits as final overrides
		}
	...
	private static void checkGuild(Guild o1, Guild o2, String name) {
		Checks.check(o1.getIdLong() == o2.getIdLong(), "Specified %s is not in the same guild! (%s / %s)", name, o1, o2);
	}
}
